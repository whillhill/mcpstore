...
from mcpstore.mcp.utilities.types import MCPStoreBaseModel
...
class MCPStoreMeta(TypedDict, total=False):
    tags: list[str]
    version: str
    versions: list[str]
...
def get_mcpstore_metadata(meta: dict[str, Any] | None) -> MCPStoreMeta:
    ...
    if not meta:
        return {}
    return cast(MCPStoreMeta, meta.get("mcpstore") or meta.get("_mcpstore") or {})
...
class MCPStoreComponent(MCPStoreBaseModel):
    """Base class for MCPStore tools, prompts, resources, and resource templates."""
    ...
    def get_meta(self) -> dict[str, Any]:
        ...
        Returns a dict that always includes a `mcpstore` key containing:
        - `tags`: sorted list of component tags
        - `version`: component version (only if set)

        Internal keys (prefixed with `_`) are stripped from the mcpstore namespace.
        """
        meta = dict(self.meta) if self.meta else {}

        mcpstore_meta: MCPStoreMeta = {"tags": sorted(self.tags)}
        if self.version is not None:
            mcpstore_meta["version"] = self.version

        # Merge with upstream mcpstore meta, stripping internal keys
        if (upstream_meta := meta.get("mcpstore")) is not None:
            if not isinstance(upstream_meta, dict):
                raise TypeError("meta['mcpstore'] must be a dict")
            # Filter out internal keys (e.g., _internal used for enabled state)
            public_upstream = {
                k: v for k, v in upstream_meta.items() if not k.startswith("_")
            }
            mcpstore_meta = cast(MCPStoreMeta, public_upstream | mcpstore_meta)
        meta["mcpstore"] = mcpstore_meta
    ...
    def enable(self) -> None:
        ...
        raise NotImplementedError(
            f"Component.enable() was removed in MCPStore 3.0. "
    ...
    def disable(self) -> None:
        ...
        raise NotImplementedError(
            f"Component.disable() was removed in MCPStore 3.0. "
    ...
    def get_span_attributes(self) -> dict[str, Any]:
        ...
        """
        return {"mcpstore.mcp.component.key": self.key}
