...
class FileSystemSource(Source):
    ...
    entrypoint: str | None = Field(
        default=None,
        description="Name of server instance or factory function (a no-arg function that returns a MCPStore server)",
    )
    ...
    async def _find_server_object(self, module: Any, file_path: Path) -> Any:
        ...
        logger.error(
            ...
            "2. Specify the entrypoint name in mcpstore.mcp.json or use `file.py:object` syntax as your path.",
            extra={"file": str(file_path)},
        )
        sys.exit(1)
