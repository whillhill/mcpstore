...
This module requires mcpstore[tasks] (pydocket). It is only imported when docket is available.
"""
...
from mcpstore.mcp.utilities.versions import VersionSpec
...
async def _lookup_task_execution(
    ...
    """
    task_meta_key = docket.key(f"mcpstore:task:{session_id}:{client_task_id}")
    created_at_key = docket.key(
        f"mcpstore:task:{session_id}:{client_task_id}:created_at"
    )
    poll_interval_key = docket.key(
        f"mcpstore:task:{session_id}:{client_task_id}:poll_interval"
    )
...
async def tasks_get_handler(server: MCPStore, params: dict[str, Any]) -> GetTaskResult:
    ...
    """
    async with mcpstore_context.Context(mcpstore=server) as ctx:
...
async def tasks_result_handler(server: MCPStore, params: dict[str, Any]) -> Any:
    ...
    async with mcpstore_context.Context(mcpstore=server) as ctx:
        ...
        if isinstance(component, Tool):
            mcpstore_result = component.convert_result(raw_value)
            mcp_result = mcpstore_result.to_mcp_result()
            ...
            mcpstore_result = component.convert_result(raw_value)
            mcp_result = mcpstore_result.to_mcp_prompt_result()
            mcp_result._meta = related_task_meta  # type: ignore[attr-defined]
            return mcp_result

        elif isinstance(component, ResourceTemplate):
            mcpstore_result = component.convert_result(raw_value)
            mcp_result = mcpstore_result.to_mcp_result(component.uri_template)
            mcp_result._meta = related_task_meta  # type: ignore[attr-defined]
            return mcp_result

        elif isinstance(component, Resource):
            mcpstore_result = component.convert_result(raw_value)
            mcp_result = mcpstore_result.to_mcp_result(str(component.uri))
            mcp_result._meta = related_task_meta  # type: ignore[attr-defined]
            return mcp_result
...
async def tasks_list_handler(
    server: MCPStore, params: dict[str, Any]
) -> ListTasksResult:
