...
from mcpstore.mcp.tools.tool import AuthCheckCallable
...
class ResourceDecoratorMixin:
    ...
    def add_resource(
        ...
        Accepts either a Resource/ResourceTemplate object or a decorated function with __mcpstore__ metadata.
        """
        enabled = True
        if not isinstance(resource, (Resource, ResourceTemplate)):
            from mcpstore.mcp.decorators import get_mcpstore_meta
            ...
            meta = get_mcpstore_meta(resource)
        ...
        def decorator(fn: AnyFunction) -> Resource | ResourceTemplate | AnyFunction:
            ...
            if params and params[0] in ("self", "cls"):
                ...
                raise TypeError(
                    ...
                    f"    from mcpstore.mcp.resources import resource\n\n"
                    f"    class MyClass:\n"
                    f"        @resource('{uri}')\n"
                    f"        def {fn_name}(...):\n"
                    f"            ...\n\n"
                    ...
                    f"See https://gomcpstore.mcp.com/servers/resources#using-with-methods"
                )

            if settings.decorator_mode == "object":
    ...
    # In compatibility mode, standalone_resource always returns a component
                ...
                target.__mcpstore__ = metadata  # type: ignore[attr-defined]
